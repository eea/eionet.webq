pipeline {
  ...
  environment {
    GIT_NAME = "eionet.webq"
    SONARQUBE_TAGS = "webforms.eionet.europa.eu"
    registry = "eeacms/webq2"
    dockerImage = ''
    tagName = ''
    webqTemplate = "templates/webq"
       ...
       steps {
            script {
                ...
                withSonarQubeEnv('Sonarqube') {
                    sh "export PATH=${javaHome}/bin:${scannerHome}/bin:$PATH"
                    sh "export JAVA_HOME=${javaHome}"
                    sh '''mvn clean -B -V verify pmd:pmd pmd:cpd spotbugs:spotbugs checkstyle:checkstyle sonar:sonar -Dsonar.java.jdkHome=/var/jenkins_home/worker/tools/hudson.model.JDK/Java11/openlogic-openjdk-11.0.9.1+1-linux-x64 -Dsonar.sources=src/main/java/ -Dsonar.test.exclusions=**/src/test/** -Dsonar.coverage.exclusions=**/src/test/** -Dsonar.java.checkstyle.reportPaths=target/checkstyle-result.xml -Dsonar.java.pmd.reportPaths=target/pmd.xml -Dsonar.coverage.jacoco.xmlReportPaths=target/site/jacoco/jacoco.xml -Dsonar.java.spotbugs.reportPaths=target/spotbugsXml.xml -Dsonar.host.url=${SONAR_HOST_URL} -Dsonar.login=${SONAR_AUTH_TOKEN} -Dsonar.java.binaries=target/classes -Dsonar.projectKey=${GIT_NAME}-${GIT_BRANCH} -Dsonar.projectName=${GIT_NAME}-${GIT_BRANCH} '''
