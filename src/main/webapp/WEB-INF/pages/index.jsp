            ...
            <p>
                ...
                <select id="selectFile" name="selectFile" title="Select new webform">
                    <c:forEach var="form" items="${allWebForms}">
                        <option value="<c:url value="/startWebform?formId=${form.id}"/>">${fn:escapeXml(form.title)}</option>
                    </c:forEach>
                </select>
            </p>
            <p>
                            ...
                            <tr class="user_file">
                                ...
                                <td>
                                    ...
                                    <span class="file-download ${fileType}">
                                        ...
                                        <a href="${fn:escapeXml(downloadLink)}" onclick="hideNotDownloadedNote('${idPrefix}');" title="Download file">
                                            ${fn:escapeXml(file.name)}<img alt="Download" src="<c:url value='/images/download-file.png'/>">
                                ...
                                <td>
                                    ...
                                    <c:forEach var="webForm" items="${allWebForms}">
                                        <c:if test="${file.xmlSchema eq webForm.xmlSchema}">
                                            <div class="action"><strong><a href="<c:url value="${fn:escapeXml(webForm.webformLink)}&amp;instance=${fn:escapeXml(downloadLink)}&amp;fileId=${file.id}&amp;base_uri=${pageContext.request.contextPath}"/>">Edit
                                                with '${fn:escapeXml(webForm.title)}' web form</a></strong></div>
                                            <c:set var="isWebformAvailableForFile" value="${true}"/>
                                        </c:if>
                                    </c:forEach>
                                    <c:if test="${not isWebformAvailableForFile}">
                                        <div class="alert"><p><strong>No webforms available. Check the XML Schema of the file ['${fn:escapeXml(file.xmlSchema)}']</strong></p></div>
                                    </c:if>
                                                    ...
                                                    <c:forEach items="${file.availableConversions}" var="conversion">
                                                        <c:choose>
                                                            <c:when test="${conversion.basic}">
                                                                <li><a href="<c:url value="/download/convert?fileId=${file.id}&amp;conversionId=${fn:escapeXml(conversion.id)}"/>">${fn:escapeXml(empty conversion.description ? conversion.resultType : conversion.description)}</a></li>
                                                            </c:when>
                                                            <c:otherwise>
                                ...
                                <td>
                                    <div class="info-container">
                                        ${fn:escapeXml(humanReadableFileSize)}
                                    </div>
                                </td>
                                <td>
                                    <div class="info-container">
                                        ...
                                        <div class="info-area" id="info-area-${file.id}">
                                            ...
                                            Downloaded: <span id="${idPrefix}downloaded">
                                            <c:choose>
